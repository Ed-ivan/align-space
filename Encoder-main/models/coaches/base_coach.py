...
from config import global_config, paths_config, hyperparameters
from models.e4e.psp import pSp
from models.coaches.models_utils import toogle_grad, load_old_G
...
class BaseCoach:
    ...
    def initilize_e4e(self):
        ckpt = torch.load(paths_config.model_paths['e4e'], map_location='cpu')
        opts = ckpt['opts']
        opts['batch_size'] = hyperparameters.train_batch_size
        opts['checkpoint_path'] = paths_config.model_paths['e4e']
        opts['stylegan_size']=hyperparameters.stylegan_size
        opts = Namespace(**opts)
