...
class SemanticGenerator(nn.Module):
    def __init__(self, size=512, style_dim=512, n_mlp=8, channel_multiplier=2,
        ...
        self.n_latent =self.n_local * 2   #modefied
    ...
    def expand_latents(self, latent):
        ...
        if latent.size(1) == self.n_latent_expand:
            return latent
        #print(latent.size(1))
       # print(self.depth_layers-self.base_layers)
        ...
        for i in range(self.n_local):
            if i == 0:
                ...
                if self.depth_layers > 0:
                    ...
                    latent_expanded.append( latent[:,2*i].unsqueeze(1).repeat(1,self.depth_layers,1) )
                if self.local_layers - self.depth_layers > 0:
                    ...
                    latent_expanded.append( latent[:,2*i+1].unsqueeze(1).repeat(1,self.local_layers-self.depth_layers,1) )
            else:
                ...
                if self.depth_layers - self.base_layers > 0:
                    ...
                    latent_expanded.append( latent[:,2*i].unsqueeze(1).repeat(1,self.depth_layers-self.base_layers,1) )
                if self.local_layers - self.depth_layers > 0:
                    #latent_expanded.append( latent[:,2*i+self.base_layers+1].unsqueeze(1).repeat(1,self.local_layers-self.depth_layers,1) )
                    latent_expanded.append( latent[:,2*i+1].unsqueeze(1).repeat(1,self.local_layers-self.depth_layers,1) )
    ...
    def composite(self, feats, depths, mask=None):
        ...
        if len(self.transparent_dims) > 3:
    ...
    def forward(self,
        ...
        if conditions is not None:
            feat=feat*(1+conditions[0])+conditions[1]
        # 这个地方是需要进行修改的 ，对 feature map的层面进行修改
